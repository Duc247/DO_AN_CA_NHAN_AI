...
def start_IDS():
    ...
    def IDS(Ma_Tran_Dich, Ma_Tran, cau_hinh):
        depth = 0
        while True:
            result = DLS(Ma_Tran_Dich, Ma_Tran, depth, cau_hinh)
            if result != "cutoff" and result is not None:
                return result
            depth += 1

    def DLS(Ma_Tran_Dich, Ma_Tran, depth, cau_hinh):
        return RECURSIVE_DLS(Ma_Tran_Dich, Ma_Tran, depth, cau_hinh)
