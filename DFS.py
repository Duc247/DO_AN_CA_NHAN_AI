...
def start_DFS():
    ...
    def Ma_Tran_Ke_Tiep_DFS_1(Ma_Tran_Hien_Tai, stack, t, visited, cau_hinh):
        N = range(8)
        for j in N:
            if 1 in Ma_Tran_Hien_Tai[:, j]: continue
            Ma_Tran_Tam_Thoi = Ma_Tran_Hien_Tai.copy()
            Ma_Tran_Tam_Thoi[t][j] = 1
            if not any(
                    np.array_equal(Ma_Tran_Tam_Thoi, s) for s in stack) and Ma_Tran_Tam_Thoi.tobytes() not in visited:
                stack.append(Ma_Tran_Tam_Thoi)
                cau_hinh[Ma_Tran_Tam_Thoi.tobytes()] = (
                Ma_Tran_Hien_Tai.dtype, Ma_Tran_Hien_Tai.shape, Ma_Tran_Hien_Tai.tobytes())

    def DFS(Ma_Tran_Dich, Trang_Thai_Mang, cau_hinh, li):
        stack = []
        stack.append(Trang_Thai_Mang)  # enqueue
        visited = set()
        visited.add(Trang_Thai_Mang.tobytes())
        while len(stack) != 0:
            Ma_tran_hien_tai = stack.pop(-1)  # dequeue
            visited.add(Ma_tran_hien_tai.tobytes())
            ...
            for i in range(8):
                if 1 not in Ma_tran_hien_tai[i]:
                    t = i
                    break
            Ma_Tran_Ke_Tiep_DFS_1(Ma_tran_hien_tai, stack, t, visited, cau_hinh)
            # 1 cái sinh ra 64 trạng thái thì trạng thái cuối sẽ đc duyệt đầu tiên
            # 1 cáii sinh ra 4 trạng thái theo hàng

        return None

    m = DFS(mang_2D, mang, Cau_hinh, li)
