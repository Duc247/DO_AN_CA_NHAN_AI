...
def start_A_sao():
    ...
    def Ma_Tran_Ke_Tiep_A_sao_1(Ma_Tran_Hien_Tai, hang_doi, hang, visited, cau_hinh,li,chi_phi_1):
        N = range(8)
        global lan_vo
        for j in N:
            if 1 not in Ma_Tran_Hien_Tai[:, j]:
                ...
                chi_phi = ham_tinh_heuristic(hang,j,li) + ham_tinh_chi_phi() + chi_phi_1
                print(chi_phi)
                if Ma_Tran_Tam_Thoi.tobytes() not in visited:
                    ...
                    cau_hinh[Ma_Tran_Tam_Thoi.tobytes()] = (
                    Ma_Tran_Hien_Tai.dtype, Ma_Tran_Hien_Tai.shape, Ma_Tran_Hien_Tai.tobytes(), chi_phi,chi_phi_1,ham_tinh_heuristic(hang,j,li) , ham_tinh_chi_phi() )

    def A_sao(Ma_Tran_Dich, Trang_Thai_Mang, cau_hinh, li):
        global lan_vo
        hang_doi = []
            ...
            for i in range(8):
                if i!= 0 :
                    if 1 not in Ma_tran_hien_tai[i]:
                        for j in range(len(Ma_tran_hien_tai[i-1])):
                            if Ma_tran_hien_tai[i-1][j] == 1 :
                                t = i
                                k= j
                        break

            if  k!= -1:
                chi_phi = key - ham_tinh_heuristic(t-1,k, li)
            else:
                chi_phi =0
            Ma_Tran_Ke_Tiep_A_sao_1(Ma_tran_hien_tai, hang_doi, t, visited, cau_hinh,li,chi_phi)
        return None

    m = A_sao(mang_2D, mang, Cau_hinh, li)
    ...
    def xu_ly_cau_hinh(cau_hinh, mang_2D):
        ...
        i = []
        while t in cau_hinh:
            dtype, shape, parent, chi_phi,a,b,c = cau_hinh[t]
            ...
            if parent is None:
                break
            t = parent
        return duong_di[::-1], tong_chi_phi[::-1], p[::-1], o[::-1], i[::-1]
    cau_hinh, tong_chi_phi ,p,o,i= xu_ly_cau_hinh(Cau_hinh, mang_2D)
    ...
    while True:
        ...
        if count < len(cau_hinh):
            ...
            am.play()
            chi_phi = str(tong_chi_phi[count]) + " " + str(p[count]) + " "+ str(o[count]) + " " +str(i[count])
