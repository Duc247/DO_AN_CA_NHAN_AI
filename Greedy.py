...
def Greedy_Search():
    ...
    def Ma_Tran_Ke_Tiep_Greedy_1(Ma_Tran_Hien_Tai,hang_doi,hang,  cau_hinh):
        N = range(8)
        global lan_vo
        for j in N:
            if 1 not in Ma_Tran_Hien_Tai[:,j]:
                    ...
                    chi_phi = ham_tinh_heuristic(hang,j,li)
                    print(chi_phi)
    ...
    def Greedy(Ma_Tran_Dich, Trang_Thai_Mang,cau_hinh):
        global lan_vo
        hang_doi =[]
        ...
        while hang_doi:
            ...
            for i in range(8):
                if 1 not in Ma_tran_hien_tai[i]:
                    t = i
                    Ma_Tran_Ke_Tiep_Greedy_1(Ma_tran_hien_tai, hang_doi, t, cau_hinh)
                    break
        return None

    m = Greedy(mang_2D,mang,Cau_hinh)
