...
def start_Beam_Search():
    ...
    def ham_tinh_heuristic(hang, j, li):
        ...
        def Ma_Tran_Ke_Tiep_beam(Ma_Tran_Hien_Tai, hang, cau_hinh):
            N = range(8)
            result = []
            global lan_vo
            for j in N:
                if 1 not in Ma_Tran_Hien_Tai[:, j]:
                    ...
                    heapq.heappush(result, (chi_phi_1, lan_vo, Ma_Tran_Tam_Thoi))
                    lan_vo += 1
                    cau_hinh[Ma_Tran_Tam_Thoi.tobytes()] = (
                    Ma_Tran_Hien_Tai.dtype, Ma_Tran_Hien_Tai.shape, Ma_Tran_Hien_Tai.tobytes(), chi_phi_1)
            return result

        def beam(Ma_Tran_Dich, Trang_Thai_Mang, cau_hinh):
            global lan_vo
            hang_doi = []
            ...
            while hang_doi:
                current = []
                for _ in range(min(beam_width, len(hang_doi))):
                    current.append(heapq.heappop(hang_doi))

                new_cur = []

                for chi_phi, _, Ma_tran_hien_tai in current:
                    if np.array_equal(Ma_tran_hien_tai, Ma_Tran_Dich):
                        return Ma_tran_hien_tai

                    for i in range(8):
                        if 1 not in Ma_tran_hien_tai[i]:
                            new_cur.extend(Ma_Tran_Ke_Tiep_beam(Ma_tran_hien_tai, i, cau_hinh))
                            break
                    hang_doi = heapq.nsmallest(beam_width, new_cur)

            return None

        m = beam(mang_2D, mang, Cau_hinh)
