...
def hill_clambing_Search():
    ...
    def Ma_Tran_Ke_Tiep_hill_clambing(Ma_Tran_Hien_Tai,hang_doi,hang,  cau_hinh,chi_phi):
        N = range(8)
        global lan_vo
        for j in N:
    ...
    def hill_clambing(Ma_Tran_Dich, Trang_Thai_Mang,cau_hinh):
        global lan_vo
        hang_doi =[]
        heapq.heappush(hang_doi,(1000,lan_vo,Trang_Thai_Mang))
        lan_vo+=1
        while hang_doi:
            ...
            for i in range(8):
                if 1 not in Ma_tran_hien_tai[i]:
                    t = i
                    hang_doi =[]
                    Ma_Tran_Ke_Tiep_hill_clambing(Ma_tran_hien_tai, hang_doi, t, cau_hinh,chi_phi)
                    break
        return None

    m = hill_clambing(mang_2D,mang,Cau_hinh)
